...
- **Data sourcing:** WHO Global Health Observatory API / World Bank Open Data API. Starting with a manageable slice (16 countries, 4 indicators) rather than everything, so the analysis stays focused and finishable.
...
- **Scope creep:** global health data is enormous; the risk is trying to cover too many indicators/countries and never finishing. Mitigation: scope was fixed early (16 countries, 4 indicators) and treated as a fixed baseline, with anything else as a "future work" list.
...
- **Time consistency:** the whole point is daily visible progress; the risk is a busy day breaking the chain. Mitigation: even a small "read data, note next step" commit counts - the diary entry that day can just say so honestly.
- **Local/remote sync issues:** learned the hard way on Day 4 that Git merge conflicts can overwrite unsaved changes if resolved carelessly - now double-checking both sides of any conflict before choosing a version.

## Status
Planning and setup complete (Days 1-2). Real data pulled via the World Bank API and first bug fixed (Days 3-4). SQL work starts Day 5.
